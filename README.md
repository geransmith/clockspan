...
**Timeclock that plans the day for you.** Tap *Now* on *Clock in* and the sheet tells you when lunch must start (default: within 5 hours) and when your day ends (default: 8 hours worked + 30-minute lunch), live, re-planning if lunch runs long. Forgot to punch? Type the time: `0730` fills the hour, the minute and a sensible AM/PM, and saves. Times follow your browser's clock; *Settings → Timeclock → Time format* forces 12-hour or 24-hour. Extra out/in pairs for appointments, before or after lunch. An explicit clock-out ends the day, early or not, with a small celebration: a line and a burst of emoji, and ticking a priority gets a smaller one (*Settings → Sheet → Celebrations* turns them off; the OS "reduce motion" setting does too).

**A sticker chart, if you want one.** Off by default. Four weeks of days, each wearing a little creature for every thing it did: clocked out, lunch taken, all priorities done, a focus session logged, retrospective reviewed. Turn it on with Customize → Show.
...
- **Sticker chart.** Hidden by default: tap **Customize**, then **Show** on *Sticker chart* in the strip at the bottom. Four weeks, Monday to Sunday; every day wears a sticker for each thing it did (clocked out, lunch taken, all priorities done, a focus session logged, retrospective reviewed), with a count on top and a day that earned all five picked out. Today updates as you go. Hover a sticker for what it was for.
